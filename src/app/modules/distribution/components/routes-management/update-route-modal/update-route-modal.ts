...
@Component({
  ...
  styleUrl: './update-route-modal.css'
})
export class UpdateRouteModal implements OnInit, OnChanges {
  ...
  loadOrganizationData(): void {
    ...
    this.isLoading = true;
    console.log('🏢 [UpdateRouteModal] Cargando datos de organización:', this.route.organizationId);
    ...
    this.organizationApi.getOrganizationById(this.route.organizationId).subscribe({
      next: (response) => {
        if (response.status) {
          ...
          console.log('✅ [UpdateRouteModal] Organización cargada:', response.data.organizationName);
          console.log('📍 [UpdateRouteModal] Zonas disponibles:', this.availableZones.length);
        } else {
      ...
      error: (error) => {
        console.error('❌ [UpdateRouteModal] Error cargando organización:', error);
  ...
  addZone(): void {
    ...
    this.selectedZones.push({
      ...
      order: this.selectedZones.length + 1
    });
  ...
  onSubmit(): void {
    ...
    console.log('🚀 [UpdateRouteModal] Actualizando ruta:', request);

    this.isSaving = true;
    this.distributionApi.updateRoute(this.route.id, request).subscribe({
      next: (response) => {
        if (response && (response.success === true || response.status === true)) {
          console.log('✅ [UpdateRouteModal] Ruta actualizada exitosamente');
          this.notificationService.success('Éxito', 'Ruta actualizada correctamente');
          ...
          console.error('❌ [UpdateRouteModal] Error en respuesta:', response.message);
      ...
      error: (error) => {
        console.error('❌ [UpdateRouteModal] Error actualizando ruta:', error);
        let errorMessage = 'Error al actualizar la ruta. Intente nuevamente.';
        
        // Manejo específico de errores HTTP
        if (error.status === 0) {
          errorMessage = 'No se pudo conectar con el servidor. Verifique su conexión a internet.';
        } else if (error.status === 400) {
          errorMessage = 'Datos inválidos para actualizar la ruta. Verifique que todos los campos estén correctamente completados.';
        } else if (error.status === 401) {
          errorMessage = 'No autorizado. Por favor inicie sesión nuevamente.';
        } else if (error.status === 403) {
          errorMessage = 'Acceso denegado. No tiene permisos para actualizar rutas.';
