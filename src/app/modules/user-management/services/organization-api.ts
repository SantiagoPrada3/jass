...
export class OrganizationApi {
     ...
     getOrganizationById(organizationId: string): Observable<OrganizationResponse> {
          console.log(`🏢 [OrganizationApi] Obteniendo organización: ${organizationId}`);
     ...
     getActiveZonesByOrganization(organizationId: string): Observable<Zone[]> {
          console.log(`🏗️ [OrganizationApi] Obteniendo zonas activas para organización: ${organizationId}`);
               ...
               this.getOrganizationById(organizationId).subscribe({
                    next: (response) => {
                         if (response.status && response.data?.zones) {
                              ...
                              console.log(`✅ [OrganizationApi] ${activeZones.length} zonas activas encontradas`);
                              observer.next(activeZones);
                              observer.complete();
                         } else {
                              console.warn('⚠️ [OrganizationApi] No se encontraron zonas para la organización');
                              observer.next([]);
                              observer.complete();
                         }
                    },
                    error: (error) => {
                         console.error('❌ [OrganizationApi] Error obteniendo zonas:', error);
     ...
     getActiveStreetsByZone(organizationId: string, zoneId: string): Observable<Street[]> {
          console.log(`🛣️ [OrganizationApi] Obteniendo calles activas para zona: ${zoneId}`);
                    ...
                    next: (response) => {
                         if (response.status && response.data?.zones) {
                              ...
                              if (zone?.streets) {
                                   ...
                                   console.log(`✅ [OrganizationApi] ${activeStreets.length} calles activas encontradas en la zona`);
                                   observer.next(activeStreets);
                                   observer.complete();
                              } else {
                                   console.warn('⚠️ [OrganizationApi] No se encontraron calles para la zona');
                                   observer.next([]);
                                   observer.complete();
                              }
                         } else {
